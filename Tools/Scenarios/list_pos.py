        ...
        for code, _, _ in entries:
            ...
            for func_name, args, _ in walk_functions(code):
                if (
                    ...
                    if len(args) >= 3:
                        now_bg_pos = update_pos(
                            now_bg_pos, parse_table(args[2]), DEFAULT_BG_POS
                        )
                ...
                elif func_name == "move" and args and get_node_name(args[0]) == "bg":
                    now_bg_pos = update_pos(
                        now_bg_pos, parse_table(args[1]), DEFAULT_BG_POS
                    )
                    dict_set_add(bg_name_to_pos_set, now_bg_name, now_bg_pos)

                elif func_name == "move" and args and get_node_name(args[0]) == "cam":
                    now_cam_pos = update_pos(
                        now_cam_pos, parse_table(args[1]), DEFAULT_CAM_POS
                    )
                    dict_set_add(bg_name_to_cam_pos_set, now_bg_name, now_cam_pos)
