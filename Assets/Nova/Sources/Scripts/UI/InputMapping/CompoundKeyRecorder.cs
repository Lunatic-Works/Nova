...
namespace Nova
{
    ...
    public class CompoundKeyRecorder : MonoBehaviour, IPointerClickHandler
    {
        private static readonly Key[] AllowedKeys =
        {
            ...
            Key.NumLock, Key.NumpadEquals, Key.NumpadDivide, Key.NumpadMultiply, Key.NumpadMinus, Key.NumpadPlus, Key.NumpadEnter, Key.NumpadPeriod,
            Key.Escape, Key.Backquote, Key.Minus, Key.Equals, Key.Backslash, Key.Backspace,
            Key.Tab, Key.CapsLock, Key.LeftBracket, Key.RightBracket, Key.Enter, Key.Semicolon, Key.Quote, Key.Comma, Key.Period, Key.Slash, Key.Space,
